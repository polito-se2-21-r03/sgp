  ...
  try {
    ...
    return await models.wallet
      ...
      .then(async () => {
        await models.order
          .findAll({
            where: { clientId: req.params.id, status: "PENDING CANCELATION" },
          })
          .then(async (orders) => {
            if (orders.length <= 0) {
            ...
            return await Promise.all(
              orders.map(async (order) => {
                return await models.order_product
                  .findAll({
                          ...
                          return await models.wallet
                            ...
                            .then(async () => {
                              ...
                              return res
                                .status(200)
                                .json(
                                  "Wallet Updated and pending orders were successfully confirmed"
                                );
                            });
                        }
                      })
